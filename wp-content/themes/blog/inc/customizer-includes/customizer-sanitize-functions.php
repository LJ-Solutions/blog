 ...
 */
function blog_sanitize_checkbox( $checked ) {
    // Boolean check.
 ...
 * @since  blog 2.1
 */
function blog_sanitize_custom_css( $input ) {
	if ( $input != '' ) {
 ...
 */
function blog_sanitize_image( $image, $setting ) {
	/*
 ...
 */
function blog_sanitize_page( $input ) {
 ...
 * @since blog 2.1
 */
function blog_sanitize_category_list( $input ) {
	if ( $input != '' ) {
		$args = array(
						'type'			=> 'post',
						'child_of'      => 0,
 ...
 *                    the setting default.
 */
function blog_sanitize_number_range( $number, $setting ) {
 ...
 */
function blog_sanitize_select( $input, $setting ) {
 ...
 * @since  blog 1.0
 */
function blog_reset_typography( $input ) {
    if ( $input == 1 ) {
 ...
 * @since  blog 1.0
 */
function blog_sanitize_footer_code( $input ) {
 ...
 * @since  blog 1.0
 */
function blog_sanitize_reset_footer_content( $input ) {
    if ( $input == '1' ) {
    	//Reset Footer Editor Options
        set_theme_mod( 'footer_left_content', blog_get_default_theme_options( 'footer_left_content' ) );
    	set_theme_mod( 'footer_right_content', blog_get_default_theme_options( 'footer_right_content' ) );
    }
 ...
 */
function blog_sanitize_important_link() {
	return false;
}
 ...
 * @since  blog 2.1
 */
function blog_reset_all_settings( $input ) {
	if ( $input == 1 ) {
		//Remove all set values
		remove_theme_mods();

        // Flush out all transients
        //blog_flush_transients();
    }
    else {
        return '';
    }
